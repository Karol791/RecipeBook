...
export interface AuthReposnseData {
    kind: string;
    idToken: string;
    email: string;
    refreshToken: string;
...
export class AuthService {
    ...
    login(email :string, password: string){
        return this.http.post<AuthReposnseData>('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyA_HWB51GpImzvSbID1EdVR_8Hunk6Abp4', {
            email: email,
            password: password,
            returnSecureToken: true
        }).pipe(catchError(this.handleError), tap(resData => {
